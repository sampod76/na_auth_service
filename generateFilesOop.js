...
export class ${capitalize(folderName)}ControllerClass {
  public service: ${capitalize(folderName)}ServiceClass;
  constructor() {
    this.service = new ${capitalize(folderName)}ServiceClass();
  }

  create${capitalize(folderName)} = catchAsync(async (req: Request, res: Response) => {
    req.body = {
      ...req.body,
      author: RequestToRefUserObject(req.user as IUserRef),
    };
    const result = await this.service.create${capitalize(folderName)}ByDb(req.body, req);
  ...
  getAll${capitalize(folderName)} = catchAsync(async (req: Request, res: Response) => {
  ...
  getSingle${capitalize(folderName)} = catchAsync(async (req: Request, res: Response) => {
    const { id } = req.params;
  ...
  update${capitalize(folderName)} = catchAsync(async (req: Request, res: Response) => {
    const { id } = req.params;
  ...
  update${capitalize(folderName)}SerialNumber = catchAsync(
    async (req: Request, res: Response) => {
      const result = await this.service.update${capitalize(folderName)}SerialNumberFromDb(
        req.body,
      );
  ...
  delete${capitalize(folderName)} = catchAsync(async (req: Request, res: Response) => {
    const { id } = req.params;
...
const ${capitalize(folderName)}Validation = new ${capitalize(folderName)}ValidationClass();
...
import { ${capitalize(folderName)}Model, I${capitalize(folderName)} } from "./interface.${capitalize(folderName)}";
...
${capitalize(folderName)}Schema.post("save", async function (data: I${capitalize(folderName)}, next) {
  try {
    // const res = await redisClient.del(ENUM_REDIS_KEY.RIS_All_Categories);
...
export class ${capitalize(folderName)}RouteClass {
  public router: express.Router;
  private controller: ${capitalize(folderName)}ControllerClass;
  private validator: ${capitalize(folderName)}ValidationClass;
  constructor() {
    this.controller = new ${capitalize(folderName)}ControllerClass();
    this.validator = new ${capitalize(folderName)}ValidationClass();
    ...
    this.router
      ...
      .post(
        ...
        parseBodyData({}),
        validateRequestZod(this.validator.create${capitalize(folderName)}ZodSchema),
    ...
    this.router.route("/serialnumber-update").patch(
      ...
      this.controller.update${capitalize(folderName)}SerialNumber,
    );
    ...
    this.router
      ...
      .patch(
        ...
        uploadAwsS3Bucket.array("images"),
        parseBodyData({}),
        validateRequestZod(this.validator.update${capitalize(folderName)}ZodSchema),
...
export class ${capitalize(folderName)}ServiceClass {
  constructor() {
    // constructor
  }
  ...
  //
  update${capitalize(folderName)}SerialNumberFromDb = async (
